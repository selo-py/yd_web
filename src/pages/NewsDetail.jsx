...
export default function NewsDetail() {
  ...
  useEffect(() => {
    ...
    const loadItem = async () => {
      const foundItem = await getContentBySlug('news', slug)
    ...
    return (
      <>
        ...
        <main className="mx-auto max-w-3xl px-4 py-10">
          ...
          <Link to="/yeni-dunya-haber" className="text-brand hover:opacity-80">Listeye dön</Link>
        </main>
      </>
    )
  }
  ...
  return (
    <>
      <SEO
        ...
        keywords={`${item.title}, yeni dünya haber, teknoloji, inovasyon`}
        image={item.cover}
        url={`/yeni-dunya-haber/${slug}`}
        type="article"
        article={{
          publishedTime: new Date().toISOString(),
          author: 'YD İnovasyon',
          section: 'Yeni Dünya Haber',
          tags: ['Yeni Dünya Haber', 'Teknoloji', 'İnovasyon'],
        }}
      />
      ...
      <div className="mt-8">
        <Link to="/yeni-dunya-haber" className="text-brand dark:text-blue-400 hover:opacity-80 font-medium">← Listeye dön</Link>
      </div>
      </main>
    </>
  )
