...
export default function FailureDetail() {
  ...
  useEffect(() => {
    ...
    const loadItem = async () => {
      const foundItem = await getContentBySlug('failures', slug)
  ...
  if (!item) {
    return (
      <>
        ...
        <main className="mx-auto max-w-3xl px-4 py-10">
          <p className="text-zinc-700 dark:text-zinc-200">Kayıt bulunamadı.</p>
          <Link to="/basarisizlik-hikayeleri" className="text-brand hover:opacity-80">Listeye dön</Link>
        </main>
      </>
    )
  }
  ...
  return (
    <>
      <SEO
        ...
        keywords={`${item.title}, başarı(sızlık) hikayeleri, girişimcilik, iş dünyası`}
        image={item.cover}
        url={`/basarisizlik-hikayeleri/${slug}`}
        type="article"
        article={{
          publishedTime: new Date().toISOString(),
          author: 'YD İnovasyon',
          section: 'Başarı(sızlık) Hikayeleri',
          tags: ['Başarı(sızlık) Hikayeleri', 'Girişimcilik', 'İş Dünyası'],
        }}
      />
      ...
      <div className="mt-8">
        <Link to="/basarisizlik-hikayeleri" className="text-brand dark:text-blue-400 hover:opacity-80 font-medium">← Hikayelere dön</Link>
      </div>
      </main>
    </>
  )
